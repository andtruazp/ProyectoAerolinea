...
export const updateVuelo = async (req, res) => {
    const { id } = req.params; 
    const {id_empresa, id_servicio, id_cuidad, origen, destino, hora_salida, hora_llegada, asientos_dis, asientos_ocup, fecha_vuelos, tipo_avion, costo } = req.body
    const [result] = await pool.execute(
      'UPDATE calendario_vuelos SET id_empresa = ?, id_servicio = ?, id_cuidad = ?, origen = ?, destino = ?, hora_salida = ?, hora_llegada = ?, asientos_dis = ?, asientos_ocup = ?, fecha_vuelos = ?, tipo_avion = ?, costo = ? WHERE id_vuelos = ?',
      [id_empresa, id_servicio, origen, destino, hora_salida, asientos_dis, asientos_ocup, fecha_vuelos, tipo_avion, costo, id]
    )
    res.json('insertado')
}
